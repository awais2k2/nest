...
function UserQuiz() {
      ...
      .then((res) => {
        ...
        if (res.data.questions && res.data.questions.length > 0) {
          const initialiput = Array(
            res.data.questions[currentQuestionIndex].question.split(" ").length
          ).fill("");
  ...
  useEffect(() => {
    if (blanks) {
      ...
      // blanks.find((item, index) => {
      //   if (item) {
      //     const firstInput = document.querySelector(`.input-${index}`);
      //     console.log(firstInput);
      //     if (firstInput) {
  ...
  const handleAnswerChange = (index, value) => {
    ...
    if (matchingAnswers) {
      ...
      const nextInput = document.querySelector(`.input-${nextIndex - 1}`);
      if (nextInput) {
        nextInput.focus();
      }
    }
                ...
                <div key={ind}>
                  ...
                  <Box display="flex" flexDir="row" gap="5px">
                    {ques.question.split(" ").map((word, index) => (
                      <Box key={index}>
                        <Box>
                          {ques.answers.includes(word.toLowerCase()) ? (
                            <Box>({word})</Box>
                          ) : (
                            <Box>{word} </Box>
                          )}
